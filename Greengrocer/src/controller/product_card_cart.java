...
public class product_card_cart  {
    ...
    void to_delete(ActionEvent event) throws IOException {
        customer_cartData.deleteProduct(loggedIn.getUser_id(), product_name.getText());
    ...
    void to_submit(ActionEvent event) throws IOException {   
        ...
        customer_cart this_product = customer_cartData.getThisProductFromCart(loggedIn.getUser_id(), product_name.getText());
        products product = productData.getThisProduct(product_name.getText());
        try {
            double newKg = Double.parseDouble(p_amount.getText());
            if (newKg % 0.25 != 0 && newKg == 0) {
                ...
                double current_product_price = this_product.getPrice(); 
                double current_product_kg = this_product.getQuantity();

                double sNewPrice = (current_product_price * newKg) / current_product_kg;

                customer_cartData.deleteFromCart(loggedIn.getUser_id(), product_name.getText());
                customer_cartData.updateAmount(loggedIn.getUser_id(), product_name.getText(), sNewPrice, newKg, this_product.getP_image());
